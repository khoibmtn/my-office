...
export function QuickDossierTagPicker({ document: docItem, onUpdate, readOnly }: QuickDossierTagPickerProps) {
  const { staffId, isGuest } = useRole()
  const perms = usePermissions()
  const isReadOnly = readOnly ?? (isGuest || !perms.canEditDocument)
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          ...
          {currentTags.length === 0 && (
            <span className="text-xs text-slate-400 italic">Chưa gắn nhãn nào</span>
          )}
        </div>
